...
links.Graph.StepDate.prototype.getLabelMinor = function(date) {
    var MONTHS_SHORT = ["1月", "2月", "3月",
        "4月", "5月", "6月",
        "7月", "8月", "9月",
        "10月", "11月", "12月"];
    var DAYS_SHORT = ["星期日", "星期一", "星期二",
        "星期三", "星期四", "星期五", "星期六"];
    ...
    switch (this.scale) {
        ...
        case links.Graph.StepDate.SCALE.HOUR:
            return this.addZeros(date.getHours(), 2) + ":" + this.addZeros(date.getMinutes(), 2);
			case links.Graph.StepDate.SCALE.YEAR:         return String(date.getFullYear()) + "年 ";
			case links.Graph.StepDate.SCALE.MONTH:        return MONTHS_SHORT[date.getMonth()];   // month is zero based
			case links.Graph.StepDate.SCALE.DAY:          return String(date.getDate()) + "日 ";
        case links.Graph.StepDate.SCALE.WEEKDAY:      return  date.getDate()+ '日 ' + DAYS_SHORT[date.getDay()] ;       
        default:                                         return "";
    }
};
        ...
        if (dataPoint) {
            ...
            if (this.tooltipFormatter) {
                ...
                if (dataPoint.title) {
                    ...
                    html += '<tr><td width="40%">時間</td><td>' + dataPoint.date + '</td></tr>';
                    if (dataPoint.value != undefined) {
                        html += '<tr><td>在線人數</td><td>' + dataPoint.value.toPrecision(2) + '</td></tr>';
                    }
                }
                html += '</table>';
            }
