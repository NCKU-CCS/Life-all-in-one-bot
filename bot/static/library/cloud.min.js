...
function generate() {
	layout.font("Microsoft JhengHei").spiral(d3.select("input[name=spiral]:checked").property("value")), fontSize = d3.scale[d3.select("input[name=scale]:checked").property("value")]().range([10, 100]), tags.length && fontSize.domain([+tags[tags.length - 1].value || 1, +tags[0].value]), complete = 0, statusText.style("display", null), words = [], layout.stop().words(tags.slice(0, max = Math.min(tags.length, +d3.select("#max").property("value")))).start()
}
...
function draw(t, e) {
	...
	n.transition().duration(1e3).attr("transform", function (t) {
		...
		if (spec == 1) {
			type = 1;
			for (var count = 0; count < used.length; count++) {
				if (used[count] == t.text) {
					type = 0;
					break;
				}
			}
...
d3.select("#random-palette").on("click", function () {
		paletteJSON("http://www.colourlovers.com/api/palettes/random", {}, function (t) {
			fill.range(t[0].colors), vis.selectAll("text").style("fill", function (t) {
				return fillt.t(ext.toLowerCase())
			})
		}), d3.event.preventDefault()
	}),
	function () {
